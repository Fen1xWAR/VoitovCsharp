...
<? 
   ...
   echo mysql_error(); 
   $r = mysql_select_db("wdb3");   
   echo mysql_error(); 

  function dodb() {   // Создание базы данных
    echo "<h3>Пример. Создание базы данных wdb3</h3>";
    @mysql_query("create database wdb3");    //
    $r= mysql_error();
    if ($r=="Can't create database 'wdb3'; database exists")
    echo "Такая база данных уже существует. Выполните следующую операцию.";
    else echo mysql_error();
    echo "<br>";
  }

   function addpersons($id,$fio,$cp,$gr) {   // Создание таблицы spersons
    $d= time();    
